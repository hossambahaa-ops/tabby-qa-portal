-- Make the combined-view survey threshold tunable by admins, separately
-- from the QA-expertise threshold. Each view gets its own knob:
--   qa_expertise_config.min_surveys           → drives recalculate_qa_expertise
--   qa_expertise_config.combined_min_surveys  → drives recalculate_combined_expertise

ALTER TABLE qa_expertise_config
  ADD COLUMN IF NOT EXISTS combined_min_surveys integer NOT NULL DEFAULT 5;

CREATE OR REPLACE FUNCTION public.recalculate_combined_expertise(
  target_month text DEFAULT NULL::text,
  min_surveys_override integer DEFAULT NULL::integer
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  threshold := COALESCE(
    min_surveys_override,
    (SELECT combined_min_surveys FROM qa_expertise_config WHERE id = 1),
    5
  );
  ...
  RETURN jsonb_build_object(
    ...
    'pool', 'qas + agents',
    'rank_method', 'ROW_NUMBER with tiebreaks'
  );
END
$function$;

GRANT EXECUTE ON FUNCTION public.recalculate_combined_expertise(text, integer) TO authenticated;
-- Drop the old single-arg signature so callers consistently use the new one
DROP FUNCTION IF EXISTS public.recalculate_combined_expertise(text);

GRANT UPDATE (combined_min_surveys) ON public.qa_expertise_config TO authenticated;
