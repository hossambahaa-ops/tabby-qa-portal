-- Allow admin-tier users to call recalculate_qa_expertise from the UI.
-- The function stays SECURITY DEFINER so it can write qa_expertise; the
-- guard at the top rejects any non-cron caller without an admin-tier role,
-- which means widening EXECUTE to `authenticated` is safe.
--
-- auth.uid() IS NULL → cron / service_role: bypass guard (preserves the
-- existing automated post-CSAT-sync recompute).
-- auth.uid() set → real user, must have admin/super_admin/manager/hod.
...
DECLARE
  ...
  threshold int;
BEGIN
  -- Role guard: real users must be admin-tier. Cron/service_role calls
  -- (no JWT) bypass by virtue of auth.uid() being NULL.
