-- Spec compliance: the spec defines strength purely by percentile rank
-- (Champion = top 10%, Solid = 10-30%, None = otherwise). The previous
-- "single participant in topic → solid" guard was protective but
-- diverged from spec for sole-handler cases. Removed in both functions
-- so percentile bands hold exactly. Side effect: when a QA is the only
-- person handling a topic, they qualify as Champion (CUME_DIST = 1.0
-- = top 10%), as the spec prescribes.

CREATE OR REPLACE FUNCTION public.recalculate_qa_expertise(
...
BEGIN
  IF auth.uid() IS NOT NULL AND NOT has_role_or_above('admin'::user_role) THEN
    RAISE EXCEPTION 'permission denied: admin role required to recompute expertise'
      USING ERRCODE = '42501';
  END IF;

  threshold := COALESCE(
    min_surveys_override,
    (SELECT min_surveys FROM qa_expertise_config WHERE id = 1),
    5
  );
  ...
  LOOP
    ...
    topic_totals AS (
      SELECT topic, SUM(surveys_count)::int AS topic_total FROM aggregated GROUP BY topic
    ),
    ...
    scored AS (
      SELECT
        r.qa_email, r.topic, r.csat_score, r.surveys_count, r.percentile, r.strength,
...
-- Same removal in combined_expertise function
  ...
  LOOP
    ...
    pool AS (
      ...
      SELECT 'agent'::text, LOWER(TRIM(qa_email)),
             TRIM(regexp_replace(COALESCE(topic,''), '[\s\-–—]+$', '')),
...
GRANT EXECUTE ON FUNCTION public.recalculate_qa_expertise(text, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.recalculate_combined_expertise(text, integer) TO authenticated;

-- Re-run both to apply the spec-compliant strength assignment
SELECT 'qa' AS view, public.recalculate_qa_expertise(NULL, NULL) AS result
UNION ALL
SELECT 'combined', public.recalculate_combined_expertise(NULL, NULL);
