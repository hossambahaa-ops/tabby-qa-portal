-- agents_csat: same shape as csat_by_topic so the recompute function
-- can UNION them trivially. No FK to profiles — agents are deliberately
-- invisible to the rest of the app.
CREATE TABLE IF NOT EXISTS agents_csat (
  qa_email      text NOT NULL,
  month         text NOT NULL,
  topic         text NOT NULL,
  good          int NOT NULL DEFAULT 0,
  bad           int NOT NULL DEFAULT 0,
  surveys_count int NOT NULL DEFAULT 0,
  csat_score    numeric,
  updated_at    timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (qa_email, month, topic)
);

ALTER TABLE agents_csat ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS agents_csat_admin_read ON agents_csat;
CREATE POLICY agents_csat_admin_read ON agents_csat
  FOR SELECT TO authenticated
  USING (has_role_or_above('admin'::user_role));
-- writes happen only via the SECURITY DEFINER edge function (service_role),
-- so no INSERT/UPDATE/DELETE policy needed.

-- combined_expertise: same shape as qa_expertise. Holds per-(QA, month)
-- expertise rows where rank/percentile is computed against the merged
-- QA + agent pool with a hardcoded 5-survey-per-topic threshold.
CREATE TABLE IF NOT EXISTS combined_expertise (
  qa_email                text NOT NULL,
  month                   text NOT NULL,
  expertise_score         numeric NOT NULL DEFAULT 0,
  star_level              int NOT NULL DEFAULT 0,
  champion_topics         text[] NOT NULL DEFAULT ARRAY[]::text[],
  solid_topics            text[] NOT NULL DEFAULT ARRAY[]::text[],
  bnpl_score              numeric NOT NULL DEFAULT 0,
  card_score              numeric NOT NULL DEFAULT 0,
  universal_score         numeric NOT NULL DEFAULT 0,
  total_qualified_topics  int NOT NULL DEFAULT 0,
  topic_breakdown         jsonb NOT NULL DEFAULT '[]'::jsonb,
  calculated_at           timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (qa_email, month)
);

ALTER TABLE combined_expertise ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS combined_expertise_admin_read ON combined_expertise;
CREATE POLICY combined_expertise_admin_read ON combined_expertise
  FOR SELECT TO authenticated
  USING (has_role_or_above('admin'::user_role));

-- recalculate_combined_expertise: mirrors recalculate_qa_expertise
-- exactly, except the source pool is csat_by_topic UNION agents_csat
-- (with a 'source' marker), the survey threshold is hardcoded to 5,
-- and the OUTPUT is restricted to QA emails (the agents are part of
-- the comparison pool but never appear in the output rows).
CREATE OR REPLACE FUNCTION public.recalculate_combined_expertise(target_month text DEFAULT NULL::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  total_rows int := 0;
  per_month_rows int;
  months_processed text[] := ARRAY[]::text[];
  threshold int := 5; -- hardcoded per spec
BEGIN
  IF auth.uid() IS NOT NULL AND NOT has_role_or_above('admin'::user_role) THEN
    RAISE EXCEPTION 'permission denied: admin role required to recompute combined expertise'
      USING ERRCODE = '42501';
  END IF;
  ...
  LOOP
    WITH
    pool AS (
      SELECT 'qa'::text AS source,
             LOWER(TRIM(qa_email)) AS qa_email,
             TRIM(regexp_replace(COALESCE(topic,''), '[\s\-–—]+$', '')) AS topic,
             COALESCE(good,0)::int AS good,
             COALESCE(bad,0)::int AS bad,
    ...
    aggregated AS (
      ...
      FROM cleaned_filtered
      GROUP BY source, qa_email, topic
    ),
    qualified AS (
    ...
    ranked AS (
      SELECT
        ...
        CASE
          WHEN COUNT(*) OVER (PARTITION BY topic) <= 1 THEN 'solid'
    ...
    topic_totals AS (
      SELECT topic, SUM(surveys_count)::int AS topic_total
      FROM aggregated GROUP BY topic
    ),
    ...
    scored AS (
      SELECT
        r.source, r.qa_email, r.topic, r.csat_score, r.surveys_count,
        r.percentile, r.strength,
      ...
      FROM ranked r
      LEFT JOIN topic_tiers tt USING (topic)
    ),
    -- Per-QA aggregations: only source='qa' rows produce output entries.
    per_qa AS (
      SELECT
        qa_email,
        ROUND(SUM(contribution), 3) AS expertise_score,
    ...
    ON CONFLICT (qa_email, month) DO UPDATE SET
      ...
      champion_topics = EXCLUDED.champion_topics,
      solid_topics = EXCLUDED.solid_topics,
      bnpl_score = EXCLUDED.bnpl_score,
      card_score = EXCLUDED.card_score,
      universal_score = EXCLUDED.universal_score,
      total_qualified_topics = EXCLUDED.total_qualified_topics,
...
GRANT EXECUTE ON FUNCTION public.recalculate_combined_expertise(text) TO authenticated;
