-- Surface each QA's rank within the merged QA+Agent pool. Both columns
-- are computed by recalculate_combined_expertise from the same
-- contributions math used for QA scoring, just extended to agents
-- (their scores are computed in-memory only — agents still don't get
-- output rows in combined_expertise).
ALTER TABLE combined_expertise
  ADD COLUMN IF NOT EXISTS combined_rank int,
  ADD COLUMN IF NOT EXISTS combined_pool_size int;
...
BEGIN
  ...
  LOOP
    ...
    -- Per-participant total expertise score (across BOTH sources). Used
    -- to rank everyone in the merged pool. Agents only contribute to the
    -- ranking — their rows never get inserted into combined_expertise.
    all_scores AS (
      SELECT source, qa_email, ROUND(SUM(contribution), 3) AS expertise_score
      FROM scored
      GROUP BY source, qa_email
    ),
    -- Pool-wide rank (1 = best). Pool size = total participants with at
    -- least one qualified topic. Score-ties get the same dense_rank.
    ranked_pool AS (
      SELECT
        source, qa_email, expertise_score,
        DENSE_RANK() OVER (ORDER BY expertise_score DESC) AS combined_rank,
    ...
    final AS (
      SELECT
        ...
        rp.combined_rank,
        rp.combined_pool_size,
        NOW() AS calculated_at
      FROM all_qas a
      LEFT JOIN per_qa p USING (qa_email)
    ...
    ON CONFLICT (qa_email, month) DO UPDATE SET
      ...
      combined_pool_size = EXCLUDED.combined_pool_size,
...
-- Backfill the new columns by recomputing all months
SELECT public.recalculate_combined_expertise(NULL);
