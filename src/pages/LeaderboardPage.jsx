    ...
    <div className="page">
      ...
      {view==="individual" && (()=>{
        ...
        <div className="card">
          ...
          {compareMode && selectedEmails.size >= 2 && <div className="card" style={{padding:16,marginBottom:12,background:"var(--bg)"}}>
            <div style={{fontSize:11,color:"var(--tx3)",fontWeight:600,textTransform:"uppercase",letterSpacing:".5px",marginBottom:10}}>Comparing {selectedEmails.size} QAs</div>
            <div style={{overflowX:"auto"}}>
              <table style={{fontSize:12,width:"100%"}}>
                <thead><tr>
                  <th style={{textAlign:"left",padding:"6px 10px",color:"var(--tx3)",fontWeight:600}}>Metric</th>
                  {[...selectedEmails].map(em=>{const r=ranked.find(x=>x.qa_email?.toLowerCase()===em);return <th key={em} style={{textAlign:"center",padding:"6px 10px",fontWeight:700,minWidth:110}}>{r?nameFromEmail(r.qa_email).split(" ")[0]:em.split("@")[0]}</th>;})}
                </tr></thead>
                <tbody>
                  {[
                  ...
                  ].map(m=><tr key={m.label} style={{borderBottom:"1px solid var(--bd2)"}}>
                    <td style={{padding:"8px 10px",color:"var(--tx2)",fontWeight:500}}>{m.label}</td>
                    {[...selectedEmails].map(em=>{const r=ranked.find(x=>x.qa_email?.toLowerCase()===em);return <td key={em} style={{textAlign:"center",padding:"8px 10px",fontWeight:600}}>{r?m.getValue(r):"—"}</td>;})}
                  </tr>)}
                </tbody>
              </table>
            </div>
