...
function ScoreEntryPage(){
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // A roster QA's valid group-head is a QA lead OR a supervisor. Some
        // Sr QAs report straight to a supervisor with no lead in between
        // (e.g. george.amir / muhammad.ramadan under amer.saad) — the
        // attendance grid already treats supervisors as group-heads, so the
        // MTD table now matches instead of dropping them.
        const groupHeadSet = new Set(qaLeadSet);
        profRows.filter(p => p.role === "qa_supervisor").forEach(p => {
          const email = p.email?.toLowerCase();
          if (!email) return;
          groupHeadSet.add(email);
          const local = email.split("@")[0];
          if (email.endsWith("@tabby.ai")) groupHeadSet.add(local + "@tabby.sa");
          if (email.endsWith("@tabby.sa")) groupHeadSet.add(local + "@tabby.ai");
          groupHeadSet.add(local);
        });
        const rosterMgrValid = new Set(rosterRows.filter(r => {
          const mgr = r.manager_email?.toLowerCase();
          if (!mgr) return false;
          return groupHeadSet.has(mgr) || groupHeadSet.has(mgr.split("@")[0]);
        }).map(r => r.email?.toLowerCase()));
        const rosterEmailSet = new Set(rosterRows.map(x => x.email?.toLowerCase()).filter(Boolean));
        const filtered = rows.filter(r => {
          const em = r.qa_email?.toLowerCase();
          // Roster membership is authoritative and OVERRIDES the profile-role
          // blacklist below. Sr QAs (profile role senior_qa) and roster members
          // who also hold an elevated Pulse role (e.g. Hossam, super_admin) do
          // real QA work and belong on the scorecard — the blacklist was
          // silently dropping every one of them. Their manager must still be a
          // QA lead (rosterMgrValid).
          if (rosterEmailSet.has(em)) return rosterMgrValid.has(em);
          if (blacklist.has(em)) return false;
          // Non-roster rows: keep only those whose qa_tl is a known QA lead.
  ...
  const MTD_COLUMNS = [
    ...
    { k: "login_hours",     label: "Login hrs",   presets: ["all","perf"],         render: r => { const h = r.login_hours==null ? null : Number(r.login_hours); if (h==null || !isFinite(h)) return <span style={{color:"var(--tx3)"}}>—</span>; const tgt = 32 * Math.max(1, Number(r._rangeMonths) || 1); const done = h>=tgt; const gap = Math.max(0, tgt-h); return <span title={done ? `Productivity KPI unlocked (≥${tgt}h)` : `Needs ${gap.toFixed(1)}h more of ${tgt}h to unlock the Productivity KPI`} style={{color:done?"var(--green)":"var(--tx2)",fontWeight:done?600:400}}>{h.toFixed(1)}h{done ? " 🏆" : <span style={{color:"var(--amber)",fontWeight:500}}> (-{gap.toFixed(1)})</span>}</span>; } },
    ...
    // ABT SBS + ABT Validation (from qa_quality_tasks) — placed just before the
