...
function AdminPage(){
  ...
  return(<div><div className="page" style={{paddingBottom:0}}><div className="page-header" style={{marginBottom:16}}><div className="page-title">Admin panel</div></div>
    <div className="tab-bar" style={{marginBottom:0}}><button className={`tab-btn ${tab==="users"?"active":""}`} onClick={()=>setTab("users")}>Users</button><button className={`tab-btn ${tab==="teams"?"active":""}`} onClick={()=>setTab("teams")}>Teams</button><button className={`tab-btn ${tab==="audit"?"active":""}`} onClick={()=>setTab("audit")}>Audit trail</button><button className={`tab-btn ${tab==="feedback"?"active":""}`} onClick={()=>setTab("feedback")}>Feedback</button><button className={`tab-btn ${tab==="errors"?"active":""}`} onClick={()=>setTab("errors")}>Errors</button><button className={`tab-btn ${tab==="glossary"?"active":""}`} onClick={()=>setTab("glossary")}>Glossary</button></div></div>
    {tab==="users"&&<AdminUsersPage teams={teams}/>}{tab==="teams"&&<TeamManagementPage/>}{tab==="audit"&&<AuditTrailPage/>}{tab==="feedback"&&<AdminFeedbackPage/>}{tab==="errors"&&<AdminErrorsPage/>}{tab==="glossary"&&<div className="page"><Glossary/></div>}</div>);
}
