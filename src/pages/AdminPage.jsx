...
function AdminPage(){
  ...
  return(<div><div className="page" style={{paddingBottom:0}}><div className="page-header" style={{marginBottom:16}}><div className="page-title">Admin panel</div></div>
    ...
    {tab==="users"&&<AdminUsersPage teams={teams}/>}{tab==="teams"&&<TeamManagementPage/>}{tab==="audit"&&<AuditTrailPage/>}{tab==="surveys"&&<AdminSurveysPage/>}{tab==="feedback"&&<AdminFeedbackPage/>}{tab==="errors"&&<AdminErrorsPage/>}{tab==="glossary"&&<div className="page"><Glossary/></div>}</div>);
}
