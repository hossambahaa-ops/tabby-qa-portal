  ...
  return <>
    <div className="card" style={{marginBottom:20}}>
      ...
      {taskView==="calendar"&&(()=>{
        ...
        const renderMini=(task)=>{const pc=priorityConfig[task.priority]||priorityConfig.medium;const isDone=task.status==="done";const isOverdue=!isDone&&task.eta_date&&task.eta_date<todayStr;return <div key={task.id} className="mo-tap" onClick={()=>setSelectedTask(task)} style={{padding:"10px 14px",borderRadius:10,background:isDone?"transparent":isOverdue?"rgba(239,68,68,.06)":"var(--bg3)",border:`1px solid ${isDone?"var(--bd)":isOverdue?"rgba(239,68,68,.15)":"var(--bd2)"}`,borderLeft:`4px solid ${isDone?"var(--green)":pc.color}`,marginBottom:6,display:"flex",alignItems:"center",gap:10,opacity:isDone?.45:1,cursor:"pointer"}}>
          <button className="mo-tap" onClick={(e)=>{e.stopPropagation();toggleTaskDone(task);}} style={{width:22,height:22,borderRadius:7,border:`2px solid ${isDone?"var(--green)":pc.color}`,background:isDone?"var(--green)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,padding:0}}>
      ...
      {taskView!=="templates"&&doneTasks.length>0&&<div style={{marginTop:12}}>
        <button onClick={()=>setHideCompleted(!hideCompleted)} style={{background:hideCompleted?"transparent":"var(--bg)",border:"none",cursor:"pointer",fontSize:12,color:"var(--green)",fontWeight:600,fontFamily:"var(--font)",padding:"8px 14px",display:"flex",alignItems:"center",gap:6,width:"100%",borderRadius:8}}>
          ...
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--tx3)" strokeWidth="2" strokeLinecap="round" style={{marginLeft:"auto",transition:"transform var(--d2)",transform:hideCompleted?"rotate(-90deg)":"none"}}><path d="M6 9l6 6 6-6"/></svg>
        </button>
