-- Fix: many participants ended up with identical expertise_scores
-- because contributions sum to common discrete values (1.0, 0.7, 1.7…).
-- DENSE_RANK then returned the same rank to every tie, so the UI
-- showed "lots of QAs at #27, #45". Switching to ROW_NUMBER with a
-- four-key tiebreak chain so every participant gets a unique slot:
--
--   1) expertise_score DESC          (primary)
--   2) total_qualified_topics DESC   (more qualifications wins)
--   3) total_surveys DESC            (larger sample wins)
--   4) qa_email ASC                  (final deterministic tiebreak)
...
BEGIN
  ...
  LOOP
    ...
    -- Per-participant aggregate with tiebreak metrics built in.
    all_scores AS (
      SELECT
        source,
        qa_email,
    ...
    -- ROW_NUMBER with deterministic tiebreaks → every participant gets
    -- a unique rank, no more "many QAs at #27".
    ranked_pool AS (
      SELECT
        source, qa_email, expertise_score,
        ROW_NUMBER() OVER (
          ORDER BY expertise_score    DESC,
                   qualified_topic_count DESC,
                   total_surveys      DESC,
                   qa_email           ASC
        ) AS combined_rank,
  ...
  RETURN jsonb_build_object(
    ...
    'pool', 'qas + agents',
    'rank_method', 'ROW_NUMBER with tiebreaks (score, qualified_topics, total_surveys, email)'
  );
END
$function$;
...
-- Recompute existing months so the new tiebreak logic applies immediately.
